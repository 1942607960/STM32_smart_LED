...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagcoldled)
    {
        info->event[info->num] = EVENT_coldled;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuecoldled,(uint8_t *)&issuedData[byteFieldOffset],coldled_LEN);
        byteFieldOffset += coldled_LEN;
        dataPoints->valuecoldled = gizX2Y(coldled_RATIO,  coldled_ADDITION, elongateP0FormTmp.devStatus.valuecoldled); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagwarmled)
    {
        info->event[info->num] = EVENT_warmled;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuewarmled,(uint8_t *)&issuedData[byteFieldOffset],warmled_LEN);
        byteFieldOffset += warmled_LEN;
        dataPoints->valuewarmled = gizX2Y(warmled_RATIO,  warmled_ADDITION, elongateP0FormTmp.devStatus.valuewarmled); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuecoldled != cur->valuecoldled)
    {
        GIZWITS_LOG("valuecoldled Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagcoldled = 1;
        ret = 1;
    }
    if(last->valuewarmled != cur->valuewarmled)
    {
        GIZWITS_LOG("valuewarmled Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagwarmled = 1;
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagcoldled)
    {
        devStatusTmp.valuecoldled = gizY2X(coldled_RATIO,  coldled_ADDITION, dataPoints->valuecoldled); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuecoldled,coldled_LEN);
        byteFieldOffset += coldled_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagwarmled)
    {
        devStatusTmp.valuewarmled = gizY2X(warmled_RATIO,  warmled_ADDITION, dataPoints->valuewarmled); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuewarmled,warmled_LEN);
        byteFieldOffset += warmled_LEN;
    }
